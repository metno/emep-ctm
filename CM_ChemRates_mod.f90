! <CM_ChemRates_mod.f90 - A component of the EMEP MSC-W Chemical transport Model, version rv4.34>
!*****************************************************************************!
!*
...
module ChemRates_mod
  ...
  implicit none
  private
  character(len=*),parameter, public :: CM_schemes_ChemRates = " EmChem19a PM_VBS_EmChem19 BVOC_IsoMT1_emis Aqueous_EmChem16x Aero2017nx ShipNOx FFireInert SeaSalt DustExtended Ash PM_WoodFFuelInert BVOC_SQT_NV Pollen"
...
contains
  ...
  subroutine setChemRates()
    ...
    rct(10,:) = (IUPAC_TROE(1.0e-31*EXP(1.6*(LOG(300/TEMP))),  &
    ...
    rct(14,:) = 2.03e-16*EXP(-4.57*(LOG(300/TEMP)))*EXP(693.0*TINV)
    rct(15,:) = 1.8e-11*EXP(110.0*TINV)
    rct(16,:) = 3.45e-12*EXP(270.0*TINV)
    rct(17,:) = 4.5e-14*EXP(-1260.0*TINV)
    rct(18,:) = (IUPAC_TROE(3.6e-30*EXP(4.1*(LOG(300/TEMP))),  &
              & 1.9e-12*EXP(-0.2*(LOG(300/TEMP))),  &
              & 0.35,  &
              & M,  &
              & 0.75-1.27*LOG10(0.35)))
    rct(19,:) = (IUPAC_TROE(1.3e-3*EXP(3.5*(LOG(300/TEMP)))*EXP(-11000.0*TINV),  &
              & 9.70e14*EXP(-0.1*(LOG(300/TEMP)))*EXP(-11080.0*TINV),  &
              & 0.35,  &
              & M,  &
              & 0.75-1.27*LOG10(0.35)))
    rct(20,:) = (IUPAC_TROE(3.2e-30*EXP(4.5*(LOG(300/TEMP))),  &
              & 3.0e-11,  &
              & 0.41,  &
              & M,  &
    ...
    rct(21,:) = (KMT3(2.4e-14,  &
              ...
              & 2.7e-17,  &
              & 2199.0))
    rct(22,:) = (IUPAC_TROE(7.4e-31*EXP(2.4*(LOG(300/TEMP))),  &
              & 3.3e-11*EXP(0.3*(LOG(300/TEMP))),  &
              & 0.81,  &
              & M,  &
              & 0.75-1.27*LOG10(0.81)))
    rct(23,:) = 2.5e-12*EXP(260.0*TINV)
    rct(24,:) = (IUPAC_TROE(1.4e-31*EXP(3.1*(LOG(300/TEMP))),  &
              & 4.0e-12,  &
              & 0.4,  &
              & M,  &
              & 0.75-1.27*LOG10(0.4)))
    ...
    rct(43,:) = (IUPAC_TROE(3.28e-28*EXP(6.87*(LOG(300/TEMP))),  &
    ...
    rct(49,:) = (IUPAC_TROE(8.6e-29*EXP(3.1*(LOG(300/TEMP))),  &
    ...
    rct(52,:) = (IUPAC_TROE(8.0e-27*EXP(3.5*(LOG(300/TEMP))),  &
              & 9.0e-9*TINV,  &
              & 0.5,  &
              & M,  &
              & 0.75-1.27*LOG10(0.5)))
    ...
    rct(72,:) = (IUPAC_TROE(3.28e-28*EXP(6.87*(LOG(300/TEMP))),  &
              & 1.125e-11*EXP(1.105*(LOG(300/TEMP))),  &
              & 0.3,  &
              & M,  &
