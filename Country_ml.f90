! <Country_ml.f90 - A component of the EMEP MSC-W Chemical transport Model, version rv4.15>
!*****************************************************************************!
!*
...
ix=ix+1 
IC_AL=ix
Country( IC_AL ) = cc(  "AL ",'ALBA', 1 ,F,  1,  1  , "Albania                       " )
ix=ix+1 
IC_AT=ix
Country( IC_AT ) = cc(  "AT ",'AUST', 2 ,F,  2,  1  , "Austria                       " )
ix=ix+1 
IC_BE=ix
Country( IC_BE ) = cc(  "BE ",'BELG', 3 ,F,  3,  1  , "Belgium                       " )
ix=ix+1 
IC_BG=ix
Country( IC_BG ) = cc(  "BG ",'BULG', 4 ,F,  4,  2  , "Bulgaria                      " )
ix=ix+1 
IC_FCS=ix
...
ix=ix+1 
IC_FR=ix
Country( IC_FR ) = cc(  "FR ",'FRAN',  8 ,F,  8,  1  , "France                        " )
ix=ix+1 
IC_GDR=ix
...
ix=ix+1 
IC_GR=ix
Country( IC_GR ) = cc(  "GR ",'GREE', 11 ,F, 11,  2  , "Greece                        " )
ix=ix+1 
IC_HU=ix
Country( IC_HU ) = cc(  "HU ",'HUNG', 12 ,F, 12,  1  , "Hungary                       " )
ix=ix+1 
IC_IS=ix
Country( IC_IS ) = cc(  "IS ",'ICEL', 13 ,F, 13,  0  , "Iceland                       " )
ix=ix+1 
IC_IE=ix
Country( IC_IE ) = cc(  "IE ",'IREL', 14 ,F, 14,  0  , "Ireland                       " )
ix=ix+1 
IC_IT=ix
Country( IC_IT ) = cc(  "IT ",'ITAL', 15 ,F, 15,  1  , "Italy                         " )
ix=ix+1 
IC_LU=ix
Country( IC_LU ) = cc(  "LU ",'LUXE', 16 ,F, 16,  1  , "Luxembourg                    " )
ix=ix+1 
IC_NL=ix
Country( IC_NL ) = cc(  "NL ",'NETH', 17 ,F, 17,  1  , "Netherlands                   " )
ix=ix+1 
IC_NO=ix
Country( IC_NO ) = cc(  "NO ",'NORW', 18 ,F, 18,  1  , "Norway                        " )
ix=ix+1 
IC_PL=ix
Country( IC_PL ) = cc(  "PL ",'POLA', 19 ,F, 19,  1  , "Poland                        " )
ix=ix+1 
IC_PT=ix
Country( IC_PT ) = cc(  "PT ",'PORT', 20 ,F, 20,  0  , "Portugal                      " )
ix=ix+1 
IC_RO=ix
Country( IC_RO ) = cc(  "RO ",'ROMA', 21 ,F, 21,  2  , "Romania                       " )
ix=ix+1 
IC_ES =ix
Country( IC_ES ) = cc(  "ES ",'SPAI', 22 ,F, 22,  1  , "Spain                         " )
ix=ix+1 
IC_SE=ix
Country( IC_SE ) = cc(  "SE ",'SWED', 23 ,F, 23,  1  , "Sweden                        " )
ix=ix+1 
IC_CH=ix
Country( IC_CH ) = cc(  "CH ",'SWIT', 24 ,F, 24,  1  , "Switzerland                   " )
ix=ix+1 
IC_TR=ix
Country( IC_TR ) = cc(  "TR ",'TURK', 25 ,F, 25,  2  , "Turkey                        " )
ix=ix+1 
IC_SU=ix
Country( IC_SU ) = cc(  "SU ",'-', 26 ,F, 26,  -100  , "Former USSR                   " )
ix=ix+1 
IC_GB=ix
Country( IC_GB ) = cc(  "GB " ,'UNKI', 27 ,F, 27,  0  , "United Kingdom                " )
ix=ix+1 
IC_VUL=ix
...
ix=ix+1
IC_RUO=ix
Country( IC_RUO) = cc(  "RUO",'KOLK', 36 ,F, 36,  4  , "Kola/Karelia                  " )
ix=ix+1 
IC_RUP=ix
! Not sure about GAINS code:
Country( IC_RUP) = cc(  "RUP",'RUSS', 37 ,F, 37,  4  , "St.Petersburg/Novgorod-Pskov  " )
ix=ix+1 
IC_RUA=ix
Country( IC_RUA) = cc(  "RUA",'KALI', 38 ,F, 38,  3  , "Kaliningrad                   " )
ix=ix+1 
IC_BY=ix
Country( IC_BY ) = cc(  "BY ",'BELA', 39 ,F, 39,  3  , "Belarus                       " )
ix=ix+1 
IC_UA=ix
Country( IC_UA ) = cc(  "UA ",'UKRA', 40 ,F, 40,  2  , "Ukraine                       " )
ix=ix+1 
IC_MD=ix
Country( IC_MD ) = cc(  "MD ",'MOLD', 41 ,F, 41,  2  , "Moldova, Republic of          " )
ix=ix+1 
IC_RUR=ix
!Could also be REMR for GAINS
Country( IC_RUR) = cc(  "RUR",'RUSS', 42 ,F, 42,  4  , "Rest of Russia                " )
ix=ix+1 
IC_EE=ix
Country( IC_EE ) = cc(  "EE ",'ESTO', 43 ,F, 43,  2  , "Estonia                       " )
ix=ix+1 
IC_LV=ix
Country( IC_LV ) = cc(  "LV ",'LATV', 44 ,F, 44,  2  , "Latvia                        " )
ix=ix+1 
IC_LT=ix
Country( IC_LT ) = cc(  "LT ",'LITH', 45 ,F, 45,  2  , "Lithuania                     " )
ix=ix+1 
IC_CZ=ix
Country( IC_CZ ) = cc(  "CZ ",'CZRE', 46 ,F, 46,  1  , "Czech                         " )
ix=ix+1 
IC_SK=ix
Country( IC_SK ) = cc(  "SK ",'SKRE', 47 ,F, 47,  1  , "Slovakia                      " )
ix=ix+1 
IC_SI=ix
Country( IC_SI ) = cc(  "SI ",'SLOV', 48 ,F, 48,  1  , "Slovenia                      " )
ix=ix+1 
IC_HR=ix
Country( IC_HR ) = cc(  "HR ",'CROA', 49 ,F, 49,  1  , "Croatia                       " )
ix=ix+1 
IC_BA=ix
Country( IC_BA ) = cc(  "BA ",'BOHE', 50 ,F, 50,  1  , "Bosnia and Herzegovina        " )
ix=ix+1 
IC_CS=ix
Country( IC_CS ) = cc(  "CS ",'SEMO', 51 ,F, 51,  1  , "Serbia and Montenegro    " )
ix=ix+1 
IC_MK=ix
Country( IC_MK ) = cc(  "MK ",'MACE', 52 ,F, 52,  1  , "Macedonia, The F.Yugo.Rep. of " )
ix=ix+1 
IC_KZ=ix
Country( IC_KZ ) = cc(  "KZ ",'KAZA', 53 ,F, 53,  -100  , "Kazakstan                     " )
ix=ix+1
IC_GE=ix
Country( IC_GE ) = cc(  "GE ",'GEOR', 54 ,F, 54,  4  , "Georgia                       " )
ix=ix+1 
IC_CY=ix
Country( IC_CY ) = cc(  "CY ",'CYPR', 55 ,F, 55,  2  , "Cyprus                        " )
ix=ix+1 
IC_AM =ix
Country( IC_AM ) = cc(  "AM ",'ARME', 56 ,F, 56,  4  , "Armenia                       " )
ix=ix+1 
IC_MT=ix
Country( IC_MT ) = cc(  "MT ",'MALT', 57 ,F, 57,  1  , "Malta                         " )
ix=ix+1 
IC_ASI=ix
...
ix=ix+1 
IC_DE=ix
Country( IC_DE ) = cc(  "DE ",'GERM', 60 ,F, 60,  1  , "Germany                       " )
ix=ix+1 
IC_RU=ix
Country( IC_RU ) = cc(  "RU " ,'RUSS', 61 ,F, 61,  -100  , "Russian Federation            " )
ix=ix+1 
IC_MC=ix
...
ix=ix+1
IC_KOSO=ix
Country(IC_KOSO) = cc( "KOSO",'KOSO', 373, F, 373, -100, "Kosovo")
ix=ix+1
IC_OCEC=ix
...
ix=ix+1
IC_RUSS_EURO = ix
Country(IC_RUSS_EURO) = cc( "RUSS_EURO",'RUSS',  385 , F,   385 ,-100, "Russia:European_part")
ix=ix+1
IC_THAI_BANG = ix
