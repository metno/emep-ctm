...
module Landuse_mod
   ...
   use Par_mod, only: LIMAX, LJMAX, &
                      limax, ljmax, me
   use SmallUtils_mod, only: wordsplit, find_index, NOT_FOUND, WriteArray, trims, lefty, pad
...
! be present in the subsequent data files for phenology and DO3SE.
   ...
   type, public :: LandCov
      ...
      real, dimension(NLUMAX) :: &
         ...
         , g_sto &! For Tleaf testing !DSJ13
         , dTleaf &! For Tleaf testing !DSJ13
   ...
   logical, private, save::  have_htMap = .false.
   real, private, allocatable, dimension(:, :) :: htMap
...
contains
   ...
   subroutine InitLanduse(daynumber)
      ...
      if ( debug_proc ) write(*,*)'HTA', trim(LandCoverInputs%htMap)
      if ( LandCoverInputs%htMap /= 'NOTSET') then
         have_htMap = .true.
         allocate (htMap(LIMAX, LJMAX))
         call ReadField_CDF(LandCoverInputs%htMap, 'TreeHeight', &
                         htMap(:, :), 1, interpol='zero_order', needed=.true., &
                         debug_flag=.false., UnDef=-999.0)
         if ( debug_proc ) write(*,*)'HTB', maxval(htMap) 
      end if
      ...
      if (MasterProc) then
         ...
         do i = 1, size(LandCoverInputs%MapFile)
           if (LandCoverInputs%MapFile(i) == "NOTSET" ) exit
           write (*, *) trim(LandCoverInputs%MapFile(i))
         end do
         write (*, *) trim(LandCoverInputs%LandDefs)
         write (*, *) trim(LandCoverInputs%Do3seDefs)
         write (*, *) trim(LandCoverInputs%mapMed)
         write (*, *) trim(LandCoverInputs%htMap)
      end if
      ...
      do i = 1, limax
         do j = 1, ljmax
            ...
            do ilu = 1, LandCover(i, j)%ncodes
               ...
               LandCover(i, j)%g_sto(:) = 0.0  ! Used for TLEAF DSJ13
               LandCover(i, j)%dTleaf(:) = 0.0  ! Used for TLEAF DSJ13
...
!TODO - find better solution!!
   ...
   subroutine ReadLanduse_CDF(filefound)
      ...
      FILELOOP: do ifile = 1, nFiles
         ...
         if (MasterProc .and. fexist) then
            ...
            write (*, *) dtxt//"LANDF", ifile, fName
         end if

         call CheckStop(status /= nf90_noerr, &
                  dtxt//"LANDUSE: NOT found "//trim(fName))
         ...
         VARIDLOOP1: do varid = 1, nVariables ! all variables in file
            ...
            msg = lefty( dtxt//"checking "//trim(fShort)//': '// varname)
            ...
            if (dbgProc) write (*,*) trim(ewords(1)),',', trim(ewords(2)),',', trim(ewords(3)),',', nwords 
            !ECOSG if (ewords(3) /= "EMEP" .and. ewords(3) /= "CLM") then
            !ECOSG    if ( dbgProc ) write(*,*) dtxt//' cycles:'//trim(varname)
            !ECOSG    cycle ! ONLY EMEP and CLM coded for now
            !ECOSG end if
  ...
  !!!!!!!!!!!! ADD IAM_VEG
      ...
      do i = 1, size(OutputVegO3)
         ...
         if (j == 0) then
            ...
            if (j < 0) then ! wanted, new veg
               ...
               msg = trim(msg)//' - new IAM  LC:'
               j = 888
            else !   j>0
               !OutputVegO3(nFluxOut)%txtLC =  OutputVegO3(i)%txtLC
               msg = trim(msg)//' - more IAM LC:'
            end if
         ...
         if (dbgProc) write (*, '(a70,4i4)') lefty(trim(msg)), i, nExtraFluxVegs, nFluxOut, j
      end do
      ...
      do iam = 1, nExtraFluxVegs
         i = iam_xLC(iam)
         if (dbgProc) write (*, "(a60,3i4,L2)") lefty(dtxt//'IAM vegPOS:'//EXTRA_FLUX_VEGS(iam)), &
         ...
         do j = 1, ljmax
            ...
            do lu = 1, NLand_codes
               ...
               if (landuse_in(i, j, lu) > 0.0) then
                  ...
                  if (dbgij) write (*, "(a,2i5,2e12.4)") dtxt//'GridAll', lu, index_lu, &
            ...
            if (sumfrac < 0.99 .or. sumfrac > 1.01) then
               write (unit=errmsg, fmt="(a34,2i4,f12.4,2f7.2)") & !nb len(dtxt)=13
                  dtxt//" SumFrac Error ", i_fdom(i), j_fdom(j), sumfrac, glat(i, j), glon(i, j)
               ...
               dbgsum = 0.0
               do lu = 1, LandCover(i,j)%ncodes
                 dbgsum = dbgsum + LandCover(i,j)%fraction(lu)
                 write (*, "(a,i3,2f10.4,1x,a)") 'XSlanduse',lu,&
                         LandCover(i, j)%fraction(lu), dbgsum, LandCover(i,j)%codes(lu)
               end do
               print *, trim(errmsg)
   ...
   subroutine SetLandUse(daynumber, month)
      ...
      real :: hveg, lat_factor, tmp_lai, lai_max
      real :: xSAIadd
      integer :: pft
      logical :: dbgij, debug_sgs
      character(len=*), parameter :: dtxt = 'SetL-Use:'
...
!   mod(current_date%month+5,12)+1 in southern hemis
      ...
      if (MasterProc .and. (DEBUG%LANDUSE > 0 .or. my_first_call)) &
         write (*, "(a20,2i4,2L2)") lefty(dtxt//"New day "), daynumber, old_daynumber, USES%PFT_MAPS, GlobRun
...
!    end if
      ...
      do i = 1, limax
         do j = 1, ljmax
            ...
            effectivdaynumber = daynumber ! DEPRECATED!!
            ...
            if (dbgij) then
               write (*, "(a,i3,i6,2f7.1)") dtxt//"PHEN DATE "//print_date(), &
                  LandCover(i, j)%ncodes, daynumber, glat(i, j), glon(i, j)
            end if

            do ilu = 1, LandCover(i, j)%ncodes
               lu = LandCover(i, j)%codes(ilu)
               pft = LandType(lu)%pft
               dnam = dtxt//trim(LandDefs(lu)%code)
               ...
               !if ( dbgij ) then
               !   write(*,"(a26,i3,i5,2i4,2L3,9f8.3)") lefty(trim(dnam)//":VEGA:"),&
               !    lu, pft, daynumber, effectivdaynumber, &
               ...
               if (USES%PFT_MAPS) then
                  ...
                  if (pft > 0) then !PFT OVERWRITE!
                     if (LandCoverInputs%LAIsrc == 'LPJ-raw') then
                        LandCover(i, j)%LAI(ilu) = pft_lai(i, j, pft) ! RAW lai
                     else
                  ...
                  !    write(*,"(a32,3i6,3f10.3)") lefty(trim(dnam)//" PFTS COMP? "), &
               ...
               if (LandType(lu)%is_crop) then
                  ...
                  ! LPJ-raw also had high LAI in boreal areas, so we keep this scaling
                  ...
                  if (have_htMap) hveg = htMap(i, j) * lat_factor
                  LandCover(i, j)%SAI(ilu) = LandCover(i, j)%LAI(ilu) + 1.0
                  if (dbgij)  write(*,"(a,i4,f7.2,a)") dtxt//'FstHeight:',&
                          ilu, hveg, trim(LandDefs(lu)%code)

               else if (LandType(lu)%is_seminat) then
                  if (have_htMap) then
                     !!hveg = min(0.1*htMap(i, j),3.0)  ! 20m trees give 2m shrubs
                     !hveg = max(0.3,hveg)
                     !HACK :-( LPJ-GUESS has v. high LAI eveb n at 70N. We limit this.
                     if (glat(i, j) >= 60.0)  then
                        lai_max = max(0.0, (1.0 - 0.05*(glat(i, j) - 60.0))) * LandDefs(lu)%LAImax
                        LandCover(i, j)%LAI(ilu) = min(lai_max, LandCover(i, j)%LAI(ilu))
                     end if
                     if (dbgij)  write(*,*) dtxt//'SnlHeight:', ilu, hveg,&
                       trim(LandDefs(lu)%code), LandCover(i, j)%LAI(ilu)
                  else if (glat(i, j) >= 60.0) then
                     lat_factor = max(0.3, (1.0 - 0.05*(glat(i, j) - 60.0)))
               ...
               if (dbgij)  write(*,"(a,i4,f7.2,a)") dtxt//'FinHeight:', ilu,&
                       LandCover(i,j)%hveg(ilu), trim(LandDefs(lu)%code)

               !if (debug_sgs .or. dbgij) then
               !   write (*, "(a28,i4,i6,2f7.1,2i4,3L2,2i5,9f7.2)") lefty(trim(dnam)//":VEGB:"), &
               !      lu, pft, glat(i, j), glon(i, j), daynumber, effectivdaynumber, &
               !      LandType(lu)%is_bulk, LandType(lu)%is_forest, LandType(lu)%is_crop, &
               !      LandCover(i, j)%SGS(ilu), LandCover(i, j)%EGS(ilu), &
               !      LandDefs(lu)%LAImin, LandDefs(lu)%LAImax, &
               !      LandCover(i, j)%LAI(ilu), LandCover(i, j)%fphen(ilu)
               !end if ! debug_sgs
...
! --- print out for debug cell
      if (DEBUG%LANDUSE > 0 .and. debug_proc) then
         ...
         do ilu = 1, LandCover(i, j)%ncodes
            ...
            pft = LandType(lu)%pft
            if (LandType(lu)%is_bulk) then
               write (*, "(a)") pad(dtxt//"phen cycles"//trim(LandDefs(lu)%code), 40)
               cycle    !else Growing veg present:
            end if

            if (my_first_call .and. ilu == 1) &
              write (*, "(a26,a3,a5,2a4, 3a2,2a5,a7,9a8)")&
               lefty(dtxt//"PHEN:CODE"), "LC", "pft", "jd", "xjd", "B", "F", "C",&
                "SGS", "EGS", "h", "Lmin", "Lmax", "LAI", "SAI", "fphen"
            write (*, "(a26,i3,i5,2i4, 3L2,2i5,f7.2,9f8.3)") lefty(dtxt//"PHEN:"// LandDefs(lu)%code),&
               lu, pft, daynumber, effectivdaynumber, &
               LandType(lu)%is_bulk, LandType(lu)%is_forest, LandType(lu)%is_crop, &
               LandCover(i, j)%SGS(ilu), LandCover(i, j)%EGS(ilu), &
               LandCover(i, j)%hveg(ilu), LandDefs(lu)%LAImin, LandDefs(lu)%LAImax, &
               LandCover(i, j)%LAI(ilu), LandCover(i, j)%SAI(ilu), &
