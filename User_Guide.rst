...
also include a met file for January 1:math:`^{st}` 2014. The last zipped
input file (called `other_input_files.tar.bz2`) contains all other input
files needed for running the EMEP/MSC-W model, except the aircraft
emissions, `AircraftEmis_FL.nc`, and the forest fire emissions,
`FINN_ForestFireEmis_2014.nc`. See sections [emisair] and [emisff] for
details about these emissions data.

After unzipping all the meteo tar files, the meteorology are placed
under a catalogue called ``EMEP_MSC-W_model.OpenSource2016/``. The
met/ catalogue should be moved over to current version of model, for
rv4.10: **EMEP\_MSC-W\_model.rv4.8.OpenSource**. So now there will be two
...
MM: month, DD: day, SS: seasons, POLL: pollutant type (NH:math:`_3`\ ,
CO, NO:math:`_x`\ , SO:math:`_x`\ , NMVOC, PM:math:`_{2.5}` and
PM:math:`_{co}`\ ). [Tab:inputdata]
...
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| New Global Ozone                       | Logan\_P.nc                                                     | netCDF :math:`^*`\ :math:`^*`   |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
...
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Road dust                              | RoadMap.nc and AVG\_SMI\_2005\_2010.nc                          | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Aircraft emissions                     | AircraftEmis\_FL.nc                                             | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Surface Pressure                       | SurfacePressure.nc                                              | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Forest Fire                            | FINN\_ForestFireEmis\_YYYY.nc                                   | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Dust files                             | Soil\_Tegen.nc                                                  | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
|                                        | SoilTypes\_IFS.nc                                               | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Emissions                              | emislist.POLL (7 files, EMEP 50km PS grid)                      | ASCII                           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
|                                        | Emis\_TNO7.nc (regional, 0.125:math:`\times`\ 0.0625 lon-lat)   | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
|                                        | Emis\_GLOB\_05.nc (global, 0.5:math:`\times`\ 0.5 lon-lat)      | netCDF:math:`\dagger`           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
...
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Time factors for hourly emissions      | HOURLY-FACS                                                     | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Emission heights                       | EmisHeights.txt                                                 | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Natural SO:math:`_2`                   | natso2MM.dat (12 files)                                         | ASCII                           |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Volcanoes                              | columnsource\_emission.csv                                      | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
|                                        | columnsource\_location.csv                                      | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Lightning emissions                    | lightningMM.dat (12 files)                                      | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Emissions speciation                   | emissplit.defaults.POLL                                         | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
|                                        | emissplit.specials.POLL                                         | ASCII:math:`^{*,\dagger}`       |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
...
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Landuse definitions                    | Inputs\_LandDefs.csv                                            | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Stomatal conductance                   | Inputs\_DO3SE.csv                                               | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Sites locations for surface output     | sites.dat                                                       | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+
| Sondes locations for vertical output   | sondes.dat                                                      | ASCII:math:`^*`                 |
+----------------------------------------+-----------------------------------------------------------------+---------------------------------+

Notes: :math:`\dagger` - optional (in most cases); :math:`^*` means
ASCII files with header. :math:`^*`\ :math:`^*` New O3 boundary
condition data in 30 levels. Can be used with ’NewLogan=.true.’ in
’BoundaryConditions\_ml.f90’.
...
meteorological fields are retrieved on a 0.1:math:`^{\circ}\times`\ 0.1:math:`^{\circ}`
longitude latitude coordinates and interpolated to 50:math:`\times`\ 50
km:math:`^2` polar-stereographic grid projection. Vertically, the fields
on 60 eta levels from the IFS model are interpolated onto the 37 EMEP
sigma levels. The meteorology is prepared into 37 sigma levels since the
model is under test for a finer vertical resolution. But the Opensource
code is released with 20 sigma levels and to make the model read the
meteorology properly, a description of the 20 vertical sigma levels is
needed. This is provided in an ascii file called ’Vertical\_levels.txt’
together with the other\_input data. The version of the IFS model used
for preparing these fields, Cycle 38r2, is documented in
http://www.ecmwf.int/research/ifsdocs/index.html. Previous years are
based on Cycle 36r1 with a resolution of 0.2:math:`^{\circ}\times`\ 0.2:math:`^{\circ}`
on a spherical grid. Meteorological fields currently used for EMEP/MSC-W
Model runs are given in Table [Tab:metinput]. Some verification and
description of these meteorological fields are given in Chapter 2 of the
...
3l3D fields - for 37 :math:`\sigma` levels
:math:`u,v` & m/s & Horizontal wind velocity components
:math:`q` & :math:`\nicefrac{kg}{kg}` & Specific humidity
:math:`\theta` & K & Potential temperature
:math:`CW` & :math:`\nicefrac{kg}{kg}` & Cloud water
:math:`CL` & % & 3D Cloud cover
:math:`cnvuf` & :math:`\nicefrac{kg}{sm^2}` & Convective updraft flux
:math:`cnvdf` & :math:`\nicefrac{kg}{sm^2}` & Convective downdraft flux
:math:`PR` & mm & Precipitation
3l2D fields - for Surface
:math:`PS` & hPa & Surface pressure
:math:`T2` & K & Temperature at 2m height
:math:`Rh2` & % & Relative humidity at 2m height
...
Biogenic VOC emission potentials (i.e. rates at 30:math:`^\circ`\ C and
full sunlight) are included for four different forest types in the
netCDF file EMEP\_EuroBVOC.nc. These emission potentials have unit
...
Inputs\_LandDefs.csv. The underlying emission potentials, land-cover
...
2005, with daily resolution, on a fine 1 km×1 km grid. We store these
data on a slightly coarser grid (0.2:math:`^\circ`\ ×0.2:math:`^\circ`\ )
globally for access by the EMEP/MSC-W model. To include forest fire
emissions set USE\_FOREST\_FIRES = .true. in “config\_emep.nml” and
download the 2012 GEOS-chem daily data
http://bai.acd.ucar.edu/Data/fire/. The data needs to be stored with
units mole/day in a netCDF file called FINN\_ForestFireEmis\_2013.nc
compatible with the **ForestFire\_ml.f90 ** module.
...
covers data for a global domain in 0.5:math:`\times`\ 0.5 degree
resolution.
...
Natural :math:`SO_2` emissions (dimethylsulfide (DMS) from sea) are
provided as monthly gridded files. The values are given at the surface
in :math:`\mu`\ g/m:math:`^2` for each grid cell in the domain.

Volcanoes
~~~~~~~~~

Emissions from volcanic passive degassing of :math:`SO_2` are included
for the active Italian volcanoes, Etna and Stromboli, based upon the
...
file "columnsource\_location.csv" contains the geographical coordinates
...
ash and :math:`SO_2` from volcanic eruptions. In addition to data for
passive degassing of :math:`SO_2`\ , the above two input files also
contain locations and emission parameters for two recent eruptions of
Icelandic volcanoes (Eyjafjallajökull in 2010 and Grimsvötn in 2011). In
...
**ASCII** format for the following compounds: CO, NH:math:`_{3}`\ ,
NO:math:`_{x}`\ , PM:math:`_{2.5}`\ , PM:math:`_{co}`\ , SO:math:`_{x}`
and VOC.
...
columns are the ‘i’ and ‘j’ indices of the EMEP grid, the fourth and
...
biogenic emission sources. The data are given with the unit: :math:`Mg`\ .
...
NH:math:`_{3}`\ , NO:math:`_{x}`\ , PM:math:`_{2.5}`\ , PM:math:`_{co}`\ ,
SO:math:`_{x}` and VOC). There is one file available per compound in
**ASCII** format.
...
pollutant (sox/nox/voc/nh3/co/pm25/pmco) from sectors of specified
...
NOx includes NO+NO:math:`_2`\ , and VOC can include many compounds. The
information needed to retreive emissions of individual compounds from
these the gridded files is given in files labelled
emissplit.defaults.:math:`poll` or emissplit.specials.:math:`poll`\ ,
where :math:`poll` can be nox, voc, etc.
...
speciation for particular countries or SNAP sectors. The 1:math:`{^st}`
...
Emissions of NO:math:`_{x}` from lightning are included in the model as
monthly averages on T21 (5.65:math:`^{\circ}\times`\ 5.65:math:`^{\circ}`\ )
resolution (Køhler *et al.*, 1995). The lightning emissions are defined
on a 64:math:`\times`\ 32 grid with 17 vertical levels, with global
...
(:math:`m`\ ), the sixth indicates albedo (%) and the seventh indicates
possible source of NH:math:`_{x}` (0 off/1 on, curently not used).
...
O:math:`_{3}` and stomatal exchange (DO3SE) must be specified. That are
...
unit: :math:`1/s`\ . For more details about these rates, please read
...
[sec:sitessondes\ :sub:`i`\ nput] The model provides a possibility for
...
For dry depositions, given per 1m:math:`^{2}` of specified landuse, the
names look like DDEP\_COMPONENT\_m2LANDUSE, where LANDUSE can be either
a specific landuse type or a cell average. The units are or . For wet
...
included in the integration. The units are molec/m:math:`^2`\ , but can
easily be changed in My\_Derived\_ml.f90 and Derived\_ml.f90.
...
SURF\_ppb\_O3 & O:math:`_{3}` [ppb]&
SURF\_ugN\_NO & NO []& Available also in ppb
 SURF\_ugN\_NO2 & NO:math:`_{2}` []& Available also in ppb
 SURF\_ugN\_HNO3 & HNO:math:`_{3}` []& Available also in ppb
SURF\_ugN\_NH3 & NH:math:`_{3}` []& Available also in ppb
SURF\_ugS\_SO2 & SO:math:`_{2}` []& Available also in ppb
& &
SURF\_ug\_SO4 & SO:math:`_{4}^{2-}` []&
SURF\_ug\_NO3\_F & NO:math:`_{3}^{-}` [] fine aerosol & As ammonium
nitrate
SURF\_ug\_NO3\_C & NO:math:`_{3}^{-}` [] coarse aerosol& Associated with
sea salt and
& & mineral dust
SURF\_ug\_TNO3 & NO:math:`_{3}^{-}` [] total & Sum of fine and coarse
nitrate
SURF\_ug\_NH4\_F & NH:math:`_{4}^{+}` [] fine aerosol& As ammonium
sulphate and
& & ammonium nitrate
...
SURF\_ug\_DUST\_SAH\_C & Saharan dust coarse []&From Boundary conditions
...
& &
SURF\_ug\_PM10 & PM:math:`_{10}` dry mass[]&
SURF\_ug\_PM10\_rh50 & PM:math:`_{10}` wet []&PM:math:`_{10}` +
& & particle water at Rh=50%
SURF\_ug\_PM25 & PM:math:`_{2.5}` dry []& Includes fine PM and
& & 27% of coarse :math:`NO_{3}`
SURF\_ug\_PM25\_rh50 & PM:math:`_{2.5}` wet []& PM:math:`_{2.5}` +
& & particle water at Rh=50%
SURF\_ug\_PM25X & PM:math:`_{2.5}` dry []& Includes fine PM and 27% of
& & coarse :math:`NO_{3}`\ , EC and OM
SURF\_ug\_PM25X\_rh50 & PM:math:`_{2.5}` []& As PM:math:`_{2.5}`\ X +
&& particle water at Rh=50%

SURF\_ug\_PMFINE & Fine PM []& Sum of all fine aerosols
SURF\_ug\_PPM25 & Primary PPM:math:`_{2.5}` []& Anthropogenic emissions
SURF\_ug\_PPM\_C & Primary coarse PM []& Anthropogenic emissions
SURF\_ug\_PM25\_FIRE & PM:math:`_{2.5}` from forest fires []& Sum of BC,
OC
& & and rest PM:math:`_{2.5}`
etc.& &
...
landuses
VG\_NO3\_F\_Grid & Dry deposition velocity of fine NO:math:`_{3}^{-}` &
Grid cell average
etc.& &
...
Base run
    ...
    in :math:`50\times50` km:math:`^2` grid.
...
the year, and for January 1:math:`^{st}` the following year (365 +1
files) is linked directly in the ’config\_emep.nml’ file. You need to
...
-  The variable ‘**h**yam’ (hybrid ‘a’ coefficient at layer midpoint),
   exists ::math:`{\rightarrow}` **eta coordinate**.

-  Level is indexed with ‘**k**’ ::math:`{\rightarrow}` **Sigma
   coordinate**

-  ‘**f**ilename\_eta’ exist ::math:`{\rightarrow}` **eta coordinate**
   derived from ‘vct’\* information in filename\_eta

-  Level indexed with ‘**l**ev’ and no ‘hyam’ or ‘filename\_eta’
   ::math:`{\rightarrow}` **pressure coordinate**
...
*hyai:math:`_{1}` = 0 hybi:math:`{_1}` =1* *hyai:math:`{_0}`
=P:math:`{_t}` hybi:math:`{_0}` = 0* *where P:math:`{_t}` → Pressure at
top.*
...
#. :math:`{\rightarrow}` Variable name in EMEP MSC-W model.

#. :math:`{\rightarrow}` Variable name in the External BC data file.

#. :math:`{\rightarrow}` External BC component to EMEP component
   fraction.

#. :math:`{\rightarrow}` Is this component wanted or not. (should be T).

#. :math:`{\rightarrow}` Was the BC variable found on the file (reset by
   the model)

#. :math:`{\rightarrow}` Index of the advected model variable (reset by
   the model)
...
**gSm:math:`^{3}`\ , ugNm:math:`^{3}`\ , ugCm:math:`^{3}`\ ,
ugm:math:`^{3}`\ , ppb**
...
of sox (SO:math:`_x`\ ), nox (NO:math:`_x`\ ), voc (VOC), nh3
(NH:math:`_3`\ ) and pm25 (PM:math:`_{2.5}`\ ) from all sectors in the
UK.
...
-  The first column of the second line represents the country code. (27
   ...
   Fortran module **Country\_ml.f90**. Please note that the country code
