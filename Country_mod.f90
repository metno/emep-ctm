! <Country_mod.f90 - A component of the EMEP MSC-W Chemical Transport Model, version v5.8>
!*****************************************************************************!
!*
...
module Country_mod
   ...
   use SmallUtils_mod, only: find_index, str_replace, blank_replace
   implicit none
   private
...
!QUer AL, HR, CS.....
   ...
   type, public :: cc
      ...
      character(len=80) :: name          ! name of country/region
   end type cc
...
Country(IC_BLAS_o12m) = cc("BLAS_o12m" ,'-', 519 ,T, 34,  34,  2  , "Black Sea outside 12 miles" )
   ...
   subroutine self_test()
      ...
      !do ic = 1, size(emepcc)
      !  if ( emepcc(ic) /= '-' ) then
      !    print *, 'EMEP ', ic, emepcc(ic), find_index(Country(ic)%code,EU27)
      !  end if
      !end do
